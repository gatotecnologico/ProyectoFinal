...
export const encuentraCompras = async (id: number) => {
  try {
    const [results] = await conexion.query(
      "SELECT * FROM compras WHERE id = ? LIMIT 1",
      id,
    );
    return results;
  } catch (error) {
...
export const agregarCompra = async (nuevo: CompraNueva) => {
  try {
    ...
    const [results] = await conexion.query(
      "INSERT INTO compras(idArticulo, cantidad, precio, IVA, subtotal, total, fechaCompra) VALUES(?, ?, ?, ?, ?, ?, ?)",
      [
        nuevo.id_articulo,
        nuevo.cantidad,
        nuevo.precio,
...
export const modificarCompra = async (modificado: Compra) => {
  try {
    ...
    const [results] = await conexion.query(
      "UPDATE compras SET `id articulo` = ?, cantidad = ?, precio = ?, IVA = ?" +
        ", subtotal = ?, total = ?, `fecha de compra` = ? WHERE id = ?",
      [
...
export const borrarCompra = async (id: number) => {
  try {
    const [results] = await conexion.query("DELETE FROM compras WHERE id = ?", [
      id,
    ]);
    return results;
  } catch (error) {
