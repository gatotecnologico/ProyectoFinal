...
export const agregarVenta = async (nuevo:VentaNueva) => {
    try {
        ...
        const [results] = await conexion.query('INSERT INTO ventas(`id articulo`, `id cliente`, cantidad, precio, IVA, subtotal' 
            + ', total, `fecha de la venta`) VALUES(?, ?, ?, ?, ?, ?, ?, ?)', [nuevo.id_articulo, nuevo.id_cliente, 
                nuevo.cantidad, nuevo.precio, nuevo.IVA, nuevo.subtotal, nuevo.total, nuevo.fechaVenta])
        return results
    } catch (error) {
...
export const modificarVenta = async (modificado:Venta) => {
    try {
        ...
        const [results] = await conexion.query('UPDATE ventas SET `id articulo` = ?, `id cliente` = ?, cantidad = ?, precio = ?, IVA = ?'
            + ', subtotal = ?, total = ?, `fecha de la venta` = ? WHERE id = ?', [modificado.id_articulo, modificado.id_cliente, 
                modificado.cantidad, modificado.precio, modificado.IVA, modificado.subtotal, modificado.total, modificado.fechaVenta, 
                modificado.id])
        return results
    } catch (error) {
